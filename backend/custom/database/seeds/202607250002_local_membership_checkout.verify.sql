...
WHERE `spu` IN ('CHMEML3LOCAL', 'CHMEML4LOCAL')
...
WHERE product.`spu` IN ('CHMEML3LOCAL', 'CHMEML4LOCAL')
  AND attr.`unique` IN ('c1a30001', 'c1a40001')
...
WHERE tenant_row.`slug` IN ('local-primary', 'local-secondary')
  ...
  AND plan_row.`plan_code` IN ('L3_ANNUAL', 'L4_ANNUAL')
