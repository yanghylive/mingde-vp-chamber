...
final class CrmebEventOrderGateway implements EventOrderGatewayInterface
{
    private const CHECKOUT_KEY_PATTERN = '/^[a-f0-9]{32}$/D';
    private const CHECKOUT_FORM_FIELD = 'chamber_event_checkout_key';
    private const ORDER_CACHE_SECONDS = 600;
    private const EVENT_VIRTUAL_TYPE = 3;

    public function assertTicketProduct(EventTicketOrderSnapshot $plan): void
    {
        try {
            ...
            foreach ([
                'is_virtual' => 1,
                'virtual_type' => self::EVENT_VIRTUAL_TYPE,
                'is_vip' => 0,
                'presale' => 0,
                'is_gift' => 0,
                'give_integral' => 0,
    ...
    public function assertOrderMatches(
        ...
        if ($this->positiveIntegerField($order, 'uid') !== $uid
            ...
            || $this->integerField($order, 'virtual_type') !== self::EVENT_VIRTUAL_TYPE
    ...
    /** Cancel an unpaid CRMEB event order and restore CRMEB stock facts. */
    public function cancelUnpaid(array $order): bool
    {
        $uid = $this->positiveIntegerField($order, 'uid');
        $orderNo = $this->stringField($order, 'order_id', 64);
        if ($this->binaryField($order, 'paid') === 1) {
            return false;
        }
        if ($this->binaryField($order, 'is_cancel') === 1 || $this->binaryField($order, 'is_del') === 1) {
            return true;
        }

        try {
            $orders = new StoreOrderServices(app()->make(StoreOrderDao::class));
            $orders->cancelOrder($orderNo, $uid);
        } catch (Throwable $exception) {
            $latest = $this->findByCheckoutKey($uid, $this->stringField($order, 'unique', 32));
            if ($latest === null
                || $this->binaryField($latest, 'paid') === 1
                || $this->binaryField($latest, 'is_cancel') !== 1) {
                throw $this->unavailable();
            }
        }
    ...
    private function assertConfirmation(array $confirmation, EventTicketOrderSnapshot $plan): void
    {
    ...
    private function assertCartGroup(array $cartGroup, EventTicketOrderSnapshot $plan): void
    {
     ...
     */
    private function assertPriceGroup($priceGroup, EventTicketOrderSnapshot $plan): void
    {
    ...
    private function assertCart(
        ...
        if (!is_array($product)
            ...
            || $this->integerField($product, 'virtual_type') !== self::EVENT_VIRTUAL_TYPE
    ...
    private function inconsistent(): MemberTransactionException
    {
        return new MemberTransactionException(
            ...
            'Event order data is inconsistent'
        );
    }
    ...
    private function unavailable(): MemberTransactionException
    {
        return new MemberTransactionException(
            503,
            'event_order_unavailable',
            'Event order service is temporarily unavailable'
        );
    }
}
